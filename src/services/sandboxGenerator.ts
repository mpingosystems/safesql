...
function semanticValue(name: string, type: string, rng: Rng): unknown {
  ...
  // Geographic state — BEFORE the name block so `home_state_name` (ends in
  // `_name`) resolves to a state, not a person — and before status. Token /
  // boundary matching on "state": whole word, state_*, *_state, or *_state_*.
  // real_estate / estate only *contain* the letters and are correctly ignored.
  const isStateCodeColumn = name === 'state_code' || name.endsWith('_state_code');
  const isStateColumn =
    name === 'state' ||
    name.startsWith('state_') ||
    name.endsWith('_state') ||
    name.includes('_state_');
